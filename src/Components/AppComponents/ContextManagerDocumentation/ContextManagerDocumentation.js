  ...
  async init() {
    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Method | Signature | Returns | Notes |","| --- | --- | --- | --- |","| `create` | `(name, initialState = {}, options = {})` | `boolean` | Options include `persist`, `storageKey`. |","| `getState` | `(name)` | `any | null` | Returns current state or `null` if missing. |","| `setState` | `(name, updater)` | `void` | **Replaces** the whole state. Pass the full next state, or `(prev) => newState`. |","| `patch` | `(name, partial)` | `void` | First-level **merge** — keeps the other fields (the \"update one field\" case). |","| `watch` | `(name, component, callback, selector?)` | `string | null` | Auto-cleanup via component sliceId. |","| `has` | `(name)` | `boolean` | Check if a context exists. |","| `destroy` | `(name)` | `boolean` | Removes a context and persisted storage. |","| `list` | `()` | `string[]` | Returns all context names. |","| `use` | `(name)` | `handle` | Handle bound to one context: `get/set/patch/watch/bind/has/destroy` without repeating the name. |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
