  ...
  async init() {
    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Method | Signature | Returns | Notes |\r","| --- | --- | --- | --- |\r","| `create` | `(name, initialState = {}, options = {})` | `boolean` | Options include `persist`, `storageKey`. |\r","| `getState` | `(name)` | `any | null` | Returns current state or `null` if missing. |\r","| `setState` | `(name, updater)` | `void` | **Replaces** the whole state. Pass the full next state, or `(prev) => newState`. |\r","| `patch` | `(name, partial)` | `void` | First-level **merge** — keeps the other fields (the \"update one field\" case). |\r","| `watch` | `(name, component, callback, selector?)` | `string | null` | Auto-cleanup via component sliceId. |\r","| `has` | `(name)` | `boolean` | Check if a context exists. |\r","| `destroy` | `(name)` | `boolean` | Removes a context and persisted storage. |\r","| `list` | `()` | `string[]` | Returns all context names. |\r","| `use` | `(name)` | `handle` | Handle bound to one context: `get/set/patch/watch/bind/has/destroy` without repeating the name. |\r"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
      ...
      }
      {
         ...
         if (container) {
            const code = await slice.build('CodeVisualizer', {
               value: "slice.context.setState('cart', { discount: 0.1 }); // ❌ replaces → items/total are gone\r\nslice.context.patch('cart', { discount: 0.1 });    // ✅ merges → keeps the rest\r",
               language: "javascript"
            });
            if ("patch vs setState") {
               const label = document.createElement('div');
               label.classList.add('code-block-title');
               label.textContent = "patch vs setState";
               container.appendChild(label);
            }
            container.appendChild(code);
         }
      }
      {
         ...
         if (container) {
            const code = await slice.build('CodeVisualizer', {
               value: "const settings = slice.context.use('settings');\r\nsettings.get();                          // current state\r\nsettings.patch({ selectedModel: 'pro' }); // merge\r\nsettings.watch(this, (s) => this.render(s));\r",
               language: "javascript"
            });
            if ("A bound handle") {
               const label = document.createElement('div');
               label.classList.add('code-block-title');
               label.textContent = "A bound handle";
               container.appendChild(label);
            }
            container.appendChild(code);
         }
      }
      {
         ...
         if (container) {
            ...
            if ("bind = watch + initial render") {
               const label = document.createElement('div');
               label.classList.add('code-block-title');
               label.textContent = "bind = watch + initial render";
               container.appendChild(label);
            }
            container.appendChild(code);
         }
