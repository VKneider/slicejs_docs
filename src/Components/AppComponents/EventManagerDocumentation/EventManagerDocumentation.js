  ...
  async init() {
    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Event | Fired by | Payload |","| --- | --- | --- |","| `router:change` | Router — once per navigation, after the new route renders | `{ to, from }` route objects — see [Routing](/Documentation/Routing) |","| `context:<name>` | ContextManager — on every `setState` for that context | the context's new state — see [ContextManager](/Documentation/Structural/ContextManager) |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Method | Signature | Notes |","| --- | --- | --- |","| `subscribe` | `(eventName, callback, options?)` | Auto-cleanup if `options.component` set |","| `subscribeOnce` | `(eventName, callback, options?)` | Auto-unsubscribe after first emit |","| `unsubscribe` | `(eventName, subscriptionId)` | Returns boolean |","| `emit` | `(eventName, data?)` | Emits to all subscribers |","| `bind` | `(component)` | Returns component-bound API |","| `register` | `(catalog)` or `(namespace, catalog)` | Declare events — see [Event Registry](/Documentation/Structural/EventRegistry) |","| `hasSubscribers` | `(eventName)` | Diagnostics only |","| `subscriberCount` | `(eventName)` | Diagnostics only |","| `clear` | `()` | Clears all subscriptions |","| `startRecording` | `()` | Activate emit history + counters (called by debugger on open) |","| `stopRecording` | `()` | Deactivate emit recording (called by debugger on close) |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
