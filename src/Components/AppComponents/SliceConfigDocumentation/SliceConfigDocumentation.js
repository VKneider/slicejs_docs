  ...
  async init() {
    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Field | Type | Default | Notes |","| --- | --- | --- | --- |","| `enabled` | `boolean` | `true` | Master log toggle. |","| `showLogs` | `object` | none | Per-level log filters. |","| `ui` | `object` | none | UI panel configuration. |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Field | Type | Default | Notes |","| --- | --- | --- | --- |","| `enabled` | `boolean` | `false` | Enables the LogViewer debug panel. |","| `shortcut` | `string` | none | Keyboard shortcut to toggle the panel (e.g. `alt+shift+l`). |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Field | Type | Notes |","| --- | --- | --- |","| `path` | `string` | Folder (relative to `src/`) where that category's components live. |","| `type` | `Visual` or `Service` | How to load a component in this category. |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
      ...
      }
      {
         ...
         if (container) {
            ...
            if ("paths.components") {
               const label = document.createElement('div');
               label.classList.add('code-block-title');
               label.textContent = "paths.components";
               container.appendChild(label);
            }
            container.appendChild(code);
         }
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Field | Type | Notes |","| --- | --- | --- |","| `mode` | `\"development\"` or `\"production\"` | Current runtime mode. |","| `env` | `object` | Key/value map of browser-exposed `SLICE_PUBLIC_*` variables. |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
