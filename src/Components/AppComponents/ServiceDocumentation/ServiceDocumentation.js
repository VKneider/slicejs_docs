  ...
  async init() {
    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Method | Signature | Returns | Notes |","| --- | --- | --- | --- |","| `getItem` | `(key)` | parsed value or `null` | Returns parsed JSON or null. |","| `setItem` | `(key, value)` | `boolean` | Returns success boolean. |","| `removeItem` | `(key)` | `boolean` | Returns success boolean. |","| `clear` | `()` | `boolean` | Clears localStorage. |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
