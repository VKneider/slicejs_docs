    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            ...
            if ("Access via controller") {
               const label = document.createElement('div');
               label.classList.add('code-block-title');
               label.textContent = "Access via controller";
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Method | Signature | Returns | Notes |","| --- | --- | --- | --- |","| `changeLanguage` | `(newLanguage)` | `boolean | void` | Applies translations for active components. |","| `setPropertiesForComponents` | `()` | `boolean | void` | Updates component props from messages. |","| `setMessages` | `(messages)` | `void` | Replaces the messages object. |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
