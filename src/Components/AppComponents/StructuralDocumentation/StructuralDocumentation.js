  ...
  async init() {
    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Method | Signature | Notes |\r","| --- | --- | --- |\r","| `navigate` | `(path, options?)` | Programmatic navigation. `{ replace: true }` replaces history. |\r","| `beforeEach` | `(to, from, next)` | Register a guard before navigation. |\r","| `afterEach` | `(to, from)` | Run logic after navigation. |\r","| `start` | `()` | Start routing immediately. |\r"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
