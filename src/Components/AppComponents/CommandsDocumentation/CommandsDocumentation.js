  ...
  async init() {
    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Flag | Type | Default | Notes |","| --- | --- | --- | --- |","| `-y, --yes [name]` | `string` | `my-slice-app` | Skip prompts; optional project name. |","| `--pm <pm>` | `pnpm \\| npm` | auto-detected | Package manager to use. When omitted, init detects it (user agent → available binaries) and only asks when ambiguous. |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Flag | Type | Default | Notes |","| --- | --- | --- | --- |","| `-y, --yes` | `boolean` | `false` | Auto-confirm package updates. Does **not** touch `api/index.js`. |","| `--cli` | `boolean` | `false` | Update CLI only. |","| `-f, --framework` | `boolean` | `false` | Update framework only. |","| `--update-api` | `boolean` | `false` | Overwrite your project `api/index.js` with the framework version (a `.bak` backup is created). Never done by default — the file may carry local changes. |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
